...
PERMANENT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
def forge_bot(name: str, bot_id: str, brain_choice: str, book_list: list, depth: int = 4, elo: int = 9999) -> bool:
    """
    Forges a new bot directly from parameters.
    Returns True if successful, False if invalid brain choice.
    """
    if not os.path.exists(DIRECTORY_OF_BOTS):
        os.makedirs(DIRECTORY_OF_BOTS)

    clean_bot_id = bot_id.lower().strip().replace(" ", "_")
    bot_data = {"name": name}
    books_str = str(book_list)
    ...
    if brain_choice == "1":
        bot_data["type"] = "script"
        bot_data["script_file"] = f"{clean_bot_id}.py"
        bot_data["function_name"] = "get_move"
        ...
        if os.path.exists(book_path):
            try:
                with open(book_path, "r", encoding="utf-8") as f:
                    book_data = json.load(f)
                    PERSONAL_OPENING_BOOK.update(book_data)
                print(f"📖 {clean_bot_id} loaded opening book: {{book_name}}.json")
            except Exception as e:
                print(f"❌ {clean_bot_id} failed to read {{book_name}}.json: {{e}}")
        else:
            print(f"⚠️ {clean_bot_id} could not find {{book_name}}.json")
    ...
    if core_fen in PERSONAL_OPENING_BOOK:
        ...
        for piece in board.get_all_pieces():
            if piece.color == ai_color and piece.position.to_notation() == move_str[:2]:
                for target_pos, move in piece.legal_moves.items():
                    if target_pos.to_notation() == move_str[2:]:
                        print(f"{clean_bot_id} played a random book move: {{move_str}}")
                        return move
...
def get_custom_move(board, ai_color):
    ...
    return random.choice(all_possible_moves)
'''
        py_filepath = os.path.join(DIRECTORY_OF_BOTS, f"{clean_bot_id}.py")
        with open(py_filepath, "w", encoding="utf-8") as py_file:
            py_file.write(python_code)
...
def get_move(board, ai_color):
    ...
    print(f"{clean_bot_id} is thinking with Stockfish (ELO: {elo}, Depth: {depth})...")
    return stockfish.get_stockfish_move(board, elo={elo}, depth={depth})
'''
        py_filepath = os.path.join(DIRECTORY_OF_BOTS, f"{clean_bot_id}.py")
        with open(py_filepath, "w", encoding="utf-8") as py_file:
        ...
        print(f"❌ Invalid brain choice '{brain_choice}' for bot {clean_bot_id}.")
        return False

    # Save the JSON ID card
    json_filepath = os.path.join(DIRECTORY_OF_BOTS, f"{clean_bot_id}.json")
    with open(json_filepath, "w", encoding="utf-8") as f:
        json.dump(bot_data, f, indent=4)
